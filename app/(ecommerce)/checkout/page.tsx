...
export default function CheckoutPage() {
    ...
    const handlePlaceOrder = async (e: FormEvent) => {
        ...
        deliveryMethod: deliveryMethod, // e.g. 'INSIDE_DHAKA' — or 'Inside Dhaka' (we map both)
        deliveryCost: deliveryCost,     // number, e.g. 70 or 130
        paymentMethod: paymentMethod,   // e.g. 'COD'
        items: cartItems,
        };

        // 🧠 এখন async logic Promise এর ভেতরে
        const promise = fetch('/api/checkout', {
            ...
            return `Order #${newOrder.orderNumber} placed successfully!`;
            });


        toast.promise(promise, {
            loading: 'Placing your order...',
            success: (message) => message, // ✅ এখানে শুধু string ফেরত দিচ্ছে
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <RadioGroup value={deliveryMethod} onValueChange={setDeliveryMethod} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <Label htmlFor="inside_dhaka" className={`flex flex-col items-center justify-center rounded-lg border-2 p-6 cursor-pointer transition-all ${deliveryMethod === 'INSIDE_DHAKA' ? 'border-primary bg-teal-50 shadow-md' : 'border-gray-200'}`}>
                                    <RadioGroupItem value="INSIDE_DHAKA" id="inside_dhaka" className="sr-only" />
                                    <Truck className="mb-3 h-6 w-6 text-primary" />
                                    Inside Dhaka
                                    <span className="font-bold mt-1">{formatCurrency(70)}</span>
                                </Label>
                                <Label htmlFor="outside_dhaka" className={`flex flex-col items-center justify-center rounded-lg border-2 p-6 cursor-pointer transition-all ${deliveryMethod === 'OUTSIDE_DHAKA' ? 'border-primary bg-teal-50 shadow-md' : 'border-gray-200'}`}>
                                    <RadioGroupItem value="OUTSIDE_DHAKA" id="outside_dhaka" className="sr-only" />
                                    <Truck className="mb-3 h-6 w-6 text-primary" />
                                    Outside Dhaka
                                    <span className="font-bold mt-1">{formatCurrency(130)}</span>
                                </Label>
                            </RadioGroup>
                        </CardContent>
                    </Card>
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <RadioGroup value={paymentMethod} onValueChange={setPaymentMethod}>
                                <Label htmlFor="cod" className={`flex items-center rounded-lg border-2 p-4 cursor-pointer transition-all ${paymentMethod === 'COD' ? 'border-primary bg-teal-50 shadow-md' : 'border-gray-200'}`}>
                                    <RadioGroupItem value="COD" id="cod" className="mr-3 h-5 w-5" />
                                    Cash on Delivery
                                </Label>
                                <Label htmlFor="digital" className="flex items-center rounded-lg border-2 p-4 cursor-not-allowed opacity-50 mt-4">
                                    <RadioGroupItem value="DIGITAL" id="digital" className="mr-3 h-5 w-5" disabled />
                    ...
                    <Card className="shadow-lg">
                        ...
                        <CardContent className="space-y-4">
                            ...
                            <Button
                                ...
                                className="w-full text-lg py-6 mt-4 bg-primary hover:bg-teal-700 shadow-lg"
                            >
